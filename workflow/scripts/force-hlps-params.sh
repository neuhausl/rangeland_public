...
# from https://github.com/CRC-FONDA/FORCE2NXF-Rangeland/blob/main/originalWF/force-hlps-params.sh
...
# pathes
sed -i "/^DIR_LOWER /cDIR_LOWER = NULL" $PARAM
sed -i "/^DIR_HIGHER /cDIR_HIGHER = results/higher_level/" $PARAM
sed -i "/^DIR_MASK /cDIR_MASK = results/preparation/mask/" $PARAM
sed -i "/^BASE_MASK /cBASE_MASK = aoi.tif" $PARAM
sed -i "/^FILE_ENDMEM /cFILE_ENDMEM = data/endmember/hostert-2003.txt" $PARAM
sed -i "/^FILE_TILE /cFILE_TILE = results/preparation/allowed_tiles.txt" $PARAM
sed -i "/^DIR_PROVENANCE /cDIR_PROVENANCE = NULL" $PARAM
